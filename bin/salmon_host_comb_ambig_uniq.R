...
#
# Description: 
# Combine Host quantification summaries (including ambig and unique metrics) into one DF
#
# Created by R. Hayward
# Date: 1st September 2020
#
# Input args:   [1] = salmon/*/aux_info/*_host_quant_ambig_uniq.sf
# Output file: "host_quant_combined_ambig_uniq.tsv"
#
#-------------------------
...
# Get file list
file_list <- list.files(path="salmon", pattern = "*host_quant_ambig_uniq.sf", recursive = T, include.dirs = T, full.names = T)
#Get sample names
sample_names <- list.files(path="salmon", pattern = "*host_quant_ambig_uniq.sf", recursive = T, include.dirs = T, full.names = T)
#just keep simple sample names
sample_namesv2 <- substring(sample_names, 8)
sample_namesv3 <- gsub('.{25}$','',sample_namesv2)
...
#Save
write.table(df.final_v2, "host_quant_combined_ambig_uniq.tsv", row.names = F, sep = "\t", quote = F)
