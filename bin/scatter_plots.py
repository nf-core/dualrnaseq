...
def scatter_plot(TPM_log, TPMs, name_1, name_2, lim_plot, organism):
    ...
    g1 = g1.annotate(
        stat, template="{stat}: {val:.4f}", stat="Pearson's r", loc="upper left", fontsize=15
    )  # {stat}: {val:.2f} (p = {p:.3g}) with p-value
...
parser.add_argument(
    "-q", "--quantification_table", metavar="<quantification_table_host>", help="Path to the quantification table"
)
...
for i in range(0, len(columns_conditions)):  # iterate over number of samples
    ...
    d1 = {
        prefix: [i]
    }  # dictionary with condition and its position in columns conditions, which is equivalent to positions in quantification table
...
for cond in condition_list:  # iterate over list of technical replicates' positions
    if len(cond) > 1:
        ...
        for com in combinations:  # iterate over sample pair combinations
            TPMs = pd.concat(
                [quantification_table[com[0]], quantification_table[com[1]]], axis=1
            )  # extract TPM values of two replicates
