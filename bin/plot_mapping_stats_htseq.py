...
def plot_mapping_stats(df_comb, no_samples, profile, x_lab, xticks_np, percentage, n_col, m):
    ...
    # save plot
    plt.savefig(
        "mapping_stats_" + profile + ".pdf", dpi=300, orientation="landscape", transparent=False, bbox_inches="tight"
    )


parser = argparse.ArgumentParser(description="""plot mapping statistics""")
parser.add_argument(
    "-i", "--input_files", metavar="<input_files>", default="*.tsv", help="Path to htseq mapping statistics tsv table "
)
...
if "trimmed_reads" in df_stats.columns:  # if statistics contain information on number of trimmed reads
    ...
    df_comb.columns = [
        "host quantified reads",
        "pathogen quantified reads",
        "host unquantified reads",
        "pathogen unquantified reads",
...
plot_mapping_stats(
    df_comb, no_samples, "samples_percentage", "[ % ]", np.arange(0, 105, step=5), percentage=True, n_col=n_col, m=0
)
...
if "trimmed_reads" in df_stats.columns:  # if no. of trimmed reads is defined
    df_comb = pd.concat(
        [
            df_stats["host_assigned_reads"],
            df_stats["pathogen_assigned_reads"],
            df_stats["unassigned_host_reads"],
            df_stats["unassigned_pathogen_reads"],
...
# plot mapping statistics
plot_mapping_stats(
    df_comb, no_samples, "samples_total_reads", "Number of reads", array_labels2, percentage=False, n_col=n_col, m=m
)
