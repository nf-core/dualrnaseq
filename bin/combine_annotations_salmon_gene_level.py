...
def combine_annotations_quant_salmon_gene(quantification_table, annotations_table, gene_attribute, organism):
    ...
    if organism == 'pathogen':
         quant_merged_table.to_csv("pathogencombined_quant_gene_level_annotations.csv",sep='\t')
    elif organism == 'host':
         quant_merged_table.to_csv("host_combined_quant_gene_level_annotations.csv",sep='\t')
