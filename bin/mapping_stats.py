...
def mapping_stats(quantification_table_path, gene_attribute, organism):
    ...
    for sample_name in quantification_table:  # iterate over columns of quantification_table
        if (
            "NumReads" in sample_name
        ):  # for each column (sample) with 'NumReads' sum up the number of reads and add into quant_sum dict.
...
parser.add_argument(
    "-q_p",
    "--quantification_table_pathogen",
    metavar="<quantification_table_pathogen>",
    default=".",
    help="Path to pathogen quantification table; Salmon and HTSeq",
)
parser.add_argument(
    "-q_h",
    "--quantification_table_host",
    metavar="<quantification_table_host>",
    default=".",
    help="Path to host quantification table; Salmon and HTSeq",
)
parser.add_argument(
    "-total_raw",
    "--total_no_raw_reads",
    metavar="<total_no_raw_reads>",
    default=".",
    help="Path to table with total number of raw reads for each sample; Salmon and STAR",
)
parser.add_argument(
    "-total_processed",
    "--total_no_processed_reads",
    metavar="<total_no_processed_reads>",
    help="Path to table with total number of processed reads by STAR or Salmon",
)
parser.add_argument(
    "-m_u",
    "--mapped_uniquely",
    metavar="<stats mapped uniquely >",
    default=".",
    help="Path to STAR mapping stats of uniquely mapped reads; STAR",
)
parser.add_argument(
    "-m_m",
    "--mapped_multi",
    metavar="<stats multi mapped >",
    default=".",
    help="Path to STAR mapping stats of multi mapped reads; STAR",
)
parser.add_argument(
    "-c_m",
    "--cross_mapped",
    metavar="<stats cross mapped >",
    default=".",
    help="Path to STAR mapping stats of cross_mapped reads; STAR",
)
parser.add_argument(
    "-star", "--star_stats", metavar="<stats star >", default=".", help="Path to mapping statistics of STAR; HTSeq"
)
parser.add_argument(
    "-star_pr",
    "--star_processed",
    metavar="<stats star_processed >",
    default=".",
    help="Path to STAR stats of processed reads; Salmon in alignment-based mode",
)
parser.add_argument(
    "-a", "--gene_attribute", default=".", help="gene attribute used in quantification; Salmon and HTSeq"
)
...
if args.tool == "salmon":
    ...
    if args.total_no_raw_reads.endswith(".tsv"):  # if tsv table is defined in total_no_raw_reads argument
        ...
        processed_reads_salmon = pd.read_csv(
            args.total_no_processed_reads, sep="\t", index_col=0, names=["processed_reads"]
        )
        # combine statistics
...
# collect statistics for Salmon alignment-based mode
    ...
    combined_total_mapped_reads = pd.read_csv(
        args.total_no_processed_reads, sep="\t", index_col=0, names=["total_mapped_reads"]
    )
    ...
    if args.total_no_raw_reads.endswith(".tsv"):
        ...
        results_df = pd.concat(
            [processed_reads_star, total_reads, combined_total_assigned_reads, combined_total_mapped_reads], axis=1
        )
        # calculate unmapped reads
        results_df["unmapped_reads"] = results_df["processed_reads"] - results_df["total_mapped_reads"]
        ...
        results_df = pd.concat(
            [processed_reads_star, combined_total_assigned_reads, combined_total_mapped_reads], axis=1
        )
        # calculate unmapped reads
        results_df["unmapped_reads"] = results_df["processed_reads"] - results_df["total_mapped_reads"]
        # calculate unassigned reads
...
# collect statistics for HTSeq
    ...
    # collect assigned pathogen reads
    pathogen_total_counts = mapping_stats(
        args.quantification_table_pathogen, args.gene_attribute, "pathogen_assigned_reads"
    )
    # collect assigned host reads
    ...
    results_df["unassigned_pathogen_reads"] = (
        results_df["pathogen_uniquely_mapped_reads"] - results_df["pathogen_assigned_reads"]
    )
    # save results
    results_df2 = results_df.sort_index()
    results_df2.to_csv(args.output_dir, sep="\t")
...
# collect statistics for STAR
    ...
    if args.total_no_raw_reads.endswith(".tsv"):
        ...
        processed_reads_star = pd.read_csv(
            args.total_no_processed_reads, sep="\t", index_col=0, names=["processed_reads"]
        )
        # combine statistics
